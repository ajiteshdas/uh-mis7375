...
//Toggle hide or show the content loaded into the iframe
document.getElementById('toggle-iframe-button').addEventListener('click', function () {
    const iframeWrapper = document.getElementById('iframe-wrapper');
    const iframe = iframeWrapper.querySelector('iframe');

    if (!iframe) {
        // Create and load the iframe if it doesn't exist
        const newIframe = document.createElement('iframe');
        newIframe.src = 'https://www.cdc.gov/vaccines/index.html';
        newIframe.title = 'Health Resources from CDC';
        newIframe.style.width = '100%';
        newIframe.style.height = '300px';
        newIframe.style.border = '1px solid #ccc';

        iframeWrapper.appendChild(newIframe);
        this.textContent = 'Hide this'; // Change button text
    } else {
        // Remove the iframe if it exists
        iframeWrapper.removeChild(iframe);
        this.textContent = 'Check Recommended Vaccines by CDC'; // Change button text
    }
});

//Implementing the Cookies
// Utility functions to set, get, and delete cookies
...
//get the cookie from user browser or computer
function getCookie(name) {
    ...
    for (const cookie of cookies) {
        ...
        if (key === name) {
            return decodeURIComponent(value); // Decode in case the value is URI-encoded
        }
    }
